...
import { sleeveCanvas } from './sleeve-art.js'
 ...
 * gramática comum. A regra aprovada é o inverso — **uma matéria e uma tinta**, com a
 * assinatura completa do cliente mudando dentro da mesma caixa óptica. As cinco
 * assinaturas reais são isoladas dos cabeçalhos que já estão em `public/works/`; o
 * Portfólio e a Miscelânea recebem lettering porque não têm marca de cliente.
 *
 * A imagem chega depois, então `aoCarregar` avisa quem tem de marcar a textura suja.
 * A capa desenha completa antes disso: quem não esperar vê o nome por inteiro, nunca
 * um quadrado vazio. `sleeve-art.js` é a mesma fonte que a bancada `capa-fit` mostra a
 * cem pixels, para a prova e a cena não divergirem.
 */
export function sleeveFor(work, index, aoCarregar) {
  return sleeveCanvas(work, index, aoCarregar)
}
