...
The dated sections moved out of `HANDOFF.md` by T-19, verbatim and in the order they had.
`HANDOFF.md` carries the state of the object; this carries how it got there — which is worth
keeping mostly for the wrong diagnosis sitting next to the right one.
...
The verdict on the first build: fonts and layout made more sense, but the way of interacting with the images was still poor. The fonts and the layout were the two things
...
The images, especially the Graecus desktop captures, came out very small in this layout.
...
The arrow should stop changing project and step through the images inside the page instead. They stepped between Works, which put the panel's largest and most obvious control
...
as *not enough length* and answered with a scrolling column. His next words were that it sat in a very cramped, closed-in space — the same complaint, about the same 200px strip.
...
- **The hub press is gone**, both wheels, and every mention with it — as asked. A control that did two unrelated jobs by
...
- **Both platters drift.** They used to drift *against the Vigil* — Sun at `-.04 * (1 - vigil)`,
  Moon at `.04 * vigil` — so the Moon stood perfectly still all day. Exactly what he saw: the Sun was right and the Moon was not. Both turn now; the Vigil only decides which leads.
...
The old trigger was a seventh detent past the end of the MOON's list. He could not find it — the eclipse could not be made to work — and a secret nobody can discover is a bug with a story
...
them his drawing. His verdict: the drawing did not resemble the reference at all. He was right
...
The Vigil toward night slowed badly whenever a jog was turned.
...
the jog designs were still wobbly afterwards. The crops are centred to the pixel and their rims
...
The ask: remove the bevel and depth from the faceplate painting.
...
Under Vigil everything was too dark; the display could throw some light onto the CDJ, and the buttons and crossfader too.
...
The jog designs were not centred, so spinning a jog felt wobbly. Two separate
...
Relief was halved throughout on the judgement of too much bevel and depth: normal strength 3.4 → 1.7, `normalScale`
...
Two asks: find out why the art still was not perfectly centred, and whether the supplied image was the cause; and add
a little bevel or indentation, so the wheels feel neither flat nor like a baked texture.
...
The art was off-centre on the platters, which made them wobble. The first cut was
...
was real physics and the whole was wrong — the crossfade felt bad; it should feel like a real fader, not bounce — because **a fader is not a free body**. Your fingers are on it the
